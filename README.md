...
10. **⑥ 新增易混淆辨析对比表**：三份资料的「概念辨析 / 边界 / 易混淆」板块在原有文字辨析后，各补一张结构化的**易混淆辨析对比表**（易混淆对象 / 关键区别 / 一句话区分），把 Agent vs Prompt·脚本·RAG·工具、上下文 vs 记忆·Prompt·训练数据·“窗口越大越好”、Skill vs Prompt·Agent·Connector·宏 等高频混淆点并列，更利于复习与考试。
11. **⑦ 自测题 + 参考答案扩充**：三份资料各新增 1 道开放自测题并补对应「参考答案（可点击展开）」——Agent 增加“反思/自我纠错为何重要”（对应 Reflexion）、上下文增加“长文档为何先做切片+检索”、Skill 增加“Skill 与 Connector 的区别及能否调用多个 Connector”。
12. **⑨ 参考链接再次补实**：在已核实链接基础上再各加 1 条真实可访问来源——Agent 加 arXiv《Reflexion》(2303.11366)、上下文加 arXiv《Longformer》(2004.05150)、Skill 加本仓库 `concept-relationship.html` 关联链接；全部链接已逐条核实可达，无伪造。
